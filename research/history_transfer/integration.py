"""Four-mode package integration. Bounded original startup; production untouched."""
from build_transfer import *
from build_swap import container_checksum
ENTRY=0x9be2
...
class BuiltGame(Machine):
    ...
    def code(self,u,address,size,data):
        ...
        if address==(self.base-PREFIX//16)*16+self.piece['publics']['transfer_bridge']:self.bridge_hits+=1
...
def integration():
    ...
    different=[i for i,row in enumerate(zip(*images)) if len(set(row))>1]
    ...
    checksum_state=[]
    for mode,image in zip(MODES,images):
    ...
    expected_ranges=set(range(ENTRY,ENTRY+6))|set(range(0x9cf1,0x9cf7))|{0xc88c,0xc88d,0xc890,0xc891}
    ...
    write_json(RESULTS/'integration.json',result);print('PASS four packaged games reach9690; differences only entry gate and verified checksum scratch',flush=True)
    return result
