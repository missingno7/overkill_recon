...
4. The set of relocations TLINK emits equals the original's. Their order is reported as
   evidence only: it depends on assembler passes, linker and EXEPACK.
...
def check_relocations(built, original):
    extra = sorted(set(built) - set(original)); missing = sorted(set(original) - set(built))
    if extra: raise AssertionError('Relocations not in the original: ' + ' '.join(f'{x:05X}' for x in extra))
    if missing: raise AssertionError('Original relocations not produced: ' + ' '.join(f'{x:05X}' for x in missing))
