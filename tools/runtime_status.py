...
def extend_reports():
    ...
    for f in functions:
        if f['address'] in reviewed:
            ...
            f.update({k:r[k] for k in ('name','category','contract','confidence','claim_axes')})
    ...
    for name in ('adlib','roland'):
        ...
        modules.append(row)
    status['optional_modules']=modules;write_json(ROOT/'metadata/status.json',status)
